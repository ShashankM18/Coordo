...
import {
  ...
  ChevronDown, Plus, LogOut, FolderKanban, Building2, MailPlus,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  useEffect(() => {
    const fetchInvites = async () => {
      try {
        ...
        if (res?.success) setInviteCount(res.invites.length);
      } catch (err) { /* ignore */ }
    };
    fetchInvites();
  ...
  const mainNav = [
    ...
    { to: '/invites', icon: MailPlus, label: 'Invitations', badge: inviteCount },
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        {mainNav.map(({ to, icon: Icon, label, badge }) => (
          ...
          >
            ...
            {badge > 0 && (
              <span className="flex h-5 items-center justify-center rounded-full bg-brand-100 px-2 text-[10px] font-bold text-brand-600">
                {badge > 9 ? '9+' : badge}
              </span>
            )}
          </NavLink>
